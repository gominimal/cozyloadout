...
# kittyview is optional, hence the guard — declining it must not leave an alias
# pointing at a binary that isn't there. `--force` on every invocation is
# deliberate: image output shouldn't hinge on kittyview's own terminal
# detection agreeing that the protocol is available.
if command -q kittyview
    alias kittyview 'kittyview --force'
end
